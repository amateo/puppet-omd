...
Puppet::Type.type(:omd_nagios_host).provide(:ruby) do
  ...
  def save_with_augeas(aug, entry)
    ...
    set_value(aug, entry, 'hostgroups', @resource[:hostgroups]) if @resource[:hostgroups]
    ...
    set_value(aug, entry, 'obsess_over_host', @resource[:obsess_over_host]) if @resource[:obsess_over_host]
    set_value(aug, entry, 'parents', @resource[:parents]) if @resource[:parents]
    ...
    set_value(aug, entry, 'statusmap_image', @resource[:statusmap_image]) if @resource[:statusmap_image]
    set_value(aug, entry, 'use', @resource[:use]) if @resource[:use]
    set_value(aug, entry, 'vrml_image', @resource[:vrml_image]) if @resource[:vrml_image]
  end
